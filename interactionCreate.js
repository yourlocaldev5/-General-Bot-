...
import { memberMeetsTier, tierLabel } from '../utils/roleTier.js';
      ...
      try {
        ...
        if (interaction.isChatInputCommand()) {
          try {
            ...
            if (command.tier && interaction.guild) {
              if (!memberMeetsTier(interaction.member, command.tier)) {
                throw createError(
                  `User lacks required tier ${command.tier} for ${interaction.commandName}`,
                  ErrorTypes.PERMISSION,
                  `This command requires the **${tierLabel(command.tier)}** role tier or higher.`,
                  withTraceContext({ commandName: interaction.commandName, requiredTier: command.tier, expected: true }, interactionTraceContext)
                );
              }
            }
