...
import { getTicketCategory } from '../utils/ticketPanel.js';
  ...
  async execute(interaction, client, args = []) {
    try {
      ...
      const modal = new ModalBuilder()
        .setCustomId(categoryKey ? `create_ticket_modal:${categoryKey}` : 'create_ticket_modal')
...
const createTicketModalHandler = {
  ...
  async execute(interaction, client, args = []) {
    try {
      ...
      const category = getTicketCategory(args[0]);
      ...
      const result = await createTicket(
        ...
        reason,
        'none',
        { extraRoleId: category?.roleId, categoryLabel: category?.label }
      );
